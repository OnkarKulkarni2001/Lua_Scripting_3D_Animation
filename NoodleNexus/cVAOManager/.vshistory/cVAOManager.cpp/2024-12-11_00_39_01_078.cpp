...
bool readPlyFile_XYZ_RGBA_Normal_UV(sModelDrawInfo& modelDrawInfo)
{
...
//    std::ifstream plyFile("assets/models/bun_zipper_res3.ply");
	...
	//	modelDrawInfo.pVertices = new sVertex_SHADER_FORMAT_xyz_rgb_N[modelDrawInfo.numberOfVertices];
	modelDrawInfo.pVertices = new sVertex_SHADER_FORMAT_xyz_RGBA_N_uv[modelDrawInfo.numberOfVertices];
	...
	for (unsigned index = 0; index != modelDrawInfo.numberOfVertices; index++)
	{
		...
		plyFile >> modelDrawInfo.pVertices[index].r;
		plyFile >> modelDrawInfo.pVertices[index].g;
		plyFile >> modelDrawInfo.pVertices[index].b;
		plyFile >> a;
		// Set all the vertices to white (1,1,1)
		//modelDrawInfo.pVertices->r = 1.0f;
		//modelDrawInfo.pVertices->g = 1.0f;
...
bool cVAOManager::LoadModelIntoVAO(
	    ...
	    unsigned int shaderProgramID, bool hasRGBA)
...
//	}
	if (hasRGBA) {
		if (!readPlyFile_XYZ_RGBA_Normal_UV(drawInfo))
			return false;
	}
	else {
